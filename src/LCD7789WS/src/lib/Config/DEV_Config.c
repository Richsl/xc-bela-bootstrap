...
void DEV_Digital_Write(UWORD Pin, UBYTE Value)
{
    ...
    //printf("WRITING pin %d to Value %d\r\n",Pin,Value);
...
void GPIO_Config(void)
{
    ...
    */
    DEV_RST_PIN = 65;//  rich PB P2.17
	DEV_DC_PIN = 27;//  rich PB P2.07
	DEV_CS_PIN = 19;//  we are runnign on SPI2.0 probably need to change this
	//DEV_BL_PIN = GPIO18;//  no backlight
    
    DEV_GPIO_Mode(DEV_RST_PIN, 1);
    DEV_GPIO_Mode(DEV_DC_PIN, 1);
    DEV_GPIO_Mode(DEV_CS_PIN, 1);
    //DEV_GPIO_Mode(DEV_BL_PIN, 1); //  no backlight
	
	//DEV_Digital_Write(DEV_BL_PIN, 1);//  no backlight
}
...
void DEV_SPI_Init(uint32_t Speed)
{
    ...
    #elif USE_DEV_LIB
        printf("DEV SPI Device at %d\r\n",Speed); 
        //DEV_HARDWARE_SPI_begin("/dev/spidev2.0");  // rich changed this for PB
        DEV_HARDWARE_SPI_beginSet("/dev/spidev2.0",0,Speed);  // rich changed this for PB
...
UBYTE DEV_ModuleInit(void)
{
    ...
    //DEV_SPI_Init(60000000);   // rich this is where the interface speed is defined  60Mhz???
    GPIO_Config();
    DEV_SPI_Init(24000000);   // rich this is where the interface speed is defined  24Mhz???
    GPIO_Config(); // seems to need this second GPIO config to avoid latching 
    return 0;
}
